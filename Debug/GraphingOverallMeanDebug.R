  ...
  if(showOverallMean == "Line"){
    ...
    P = P +
      geom_hline(data = OverallMean,
                 aes(yintercept=yMean, color = condition, linetype = "overallMeanLines"),
    ...
    P = P +
      geom_pointrange(data = OverallMean,
                      ...
                      position = position_dodge(width = max(OverallMean$tickSize)/2),padding = 0)+
      ...
      geom_rect(data = OverallMean,
                aes(xmin = tickSize/2+max(linedata$timepoint), 
                    xmax = tickSize*3/2+max(linedata$timepoint), ymin = -Inf, ymax = Inf), 
                fill = "gray", alpha = 0.2) 
  }
  ...
  if((showOverallMean == "Line") & showDataPointProp){
    ...
    P = P +
      scale_linetype_manual(name = NULL,
                            ...
                            values = c(lineTypeOverallMean,"dashed"),
