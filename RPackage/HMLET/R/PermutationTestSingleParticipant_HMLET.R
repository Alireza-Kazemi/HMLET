...
PermutationTestSingleParticipant_HMLET <- function(data, samples = 2000, threshold_t = NA){
  ...
  if(unique(data$ID)>1){
    stop("This routine is for permutation test at trial level for one participants.\n
  ...
  for(testName in unique(datSave$testName)){
    ...
    if(nrow(clusterInf)>0){
      ...
      L = data.frame(L)
      names(L) = 1:ncol(L)
      names(L) = c(names(L)[1:3],paste("perm",names(L)[-(1:3)],sep = ""))
      labels = merge(labels,L, by = c("ID","trial","condition"))
